...
func renameAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	account, err := w.AccountNumber(waddrmgr.DefaultKeyScope, cmd.OldAccount)
	if err != nil {
		return nil, err
	}
...
func getRawChangeAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	// If specified, use the provided key scope.
	if cmd.AddressType != nil {
		var err error
		scope, err = lookupKeyScope(cmd.AddressType)
		if err != nil {
...
func getTransaction(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	for _, cred := range details.Credits {
		...
		if err == nil && len(addrs) == 1 {
			...
			account, err := w.AccountOfAddress(addr)
			if err == nil {
				name, err := w.AccountName(waddrmgr.DefaultKeyScope, account)
				if err == nil {
					accountName = name
				}
			}
...
func sendFrom(icmd interface{}, w *wallet.Wallet,
	...
	account, err := w.AccountNumber(waddrmgr.DefaultKeyScope,
		cmd.FromAccount)
	if err != nil {
		return nil, err
	}
